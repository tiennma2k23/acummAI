...
import time
import copy
from agorithm.dfs_in_SLAM import *
...
def random_matrix(num_rows, num_cols, num_obs, num_dust):
    ...
    # Lấy vị trí cuối cùng cho máy hút bụi
    ...
    global resultprev, num_dust_prev  # Lưu lại ma trận trước đó
    num_dust_prev = num_dust
    resultprev = copy.deepcopy({
        'matrix': matrix,
...
def create_table_prev():
    ...
    num_dust = num_dust_prev  # Số lượng bụi

    result = copy.deepcopy(resultprev)

    obstacle_positions = {(i, j) for i in range(num_rows)
                          for j in range(num_cols) if result['matrix'][i][j] == 1}
    dust_positions = {(i, j) for i in range(num_rows)
                      for j in range(num_cols) if result['matrix'][i][j] == 2}
    ...
    global size_image
    size_image = min(600 // num_rows, 1350 // num_cols)

    vacuum_image = ImageTk.PhotoImage(Image.open(
        "C:/Users/HaiAnh/Projects/Python/acummAI/image/vacuum.png").resize((size_image, size_image), Image.LANCZOS))
    bg_image = ImageTk.PhotoImage(Image.open(
        "C:/Users/HaiAnh/Projects/Python/acummAI/image/rac.png").resize((size_image, size_image), Image.LANCZOS))
    dust_image = ImageTk.PhotoImage(Image.open(
        "C:/Users/HaiAnh/Projects/Python/acummAI/image/virus.jpg").resize((size_image, size_image), Image.LANCZOS))
    wall_image = ImageTk.PhotoImage(Image.open(
        "C:/Users/HaiAnh/Projects/Python/acummAI/image/wall.jpg").resize((size_image, size_image), Image.LANCZOS))
    visited_image = ImageTk.PhotoImage(Image.open(
        "C:/Users/HaiAnh/Projects/Python/acummAI/image/rac.jpg").resize((size_image, size_image), Image.LANCZOS))
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            ...
            cell_label = tk.Label(table_frame, image=image, borderwidth=0.1,
                                  relief='groove', width=size_image, height=size_image, bg='lightblue')
            cell_label.image = image
            cell_label.grid(row=i, column=j, padx=1, pady=1)
            cell_label.bind('<Button-1>', lambda onRightClick,
                            row=i, col=j: update_vacuum_position(row, col))
            cell_label.bind('<Button-2>', lambda onRightClick,
                            row=i, col=j: up_wall(row, col))
            cell_label.bind('<Button-3>', lambda onRightClick,
                            row=i, col=j: up_virus(row, col))
...
def create_table():
    ...
    global vacuum_image, bg_image, dust_image, wall_image, visited_image
    global size_image
    size_image = min(400 // num_rows, 1350 // num_cols)
...
def up_wall(r, c):
    global table_frame
    result['matrix'][r][c] = 1
    new_label = tk.Label(table_frame, image=wall_image, borderwidth=0.1,
                         relief='groove', width=size_image, height=size_image, bg='lightblue')
...
def up_virus(r, c):
    ...
    result['matrix'][r][c] = 2
    new_label = tk.Label(table_frame, image=dust_image, borderwidth=0.1,
                         relief='groove', width=size_image, height=size_image, bg='lightblue')
    # new_label.image = dust_image
    new_label.grid(row=r, column=c, padx=1, pady=1)
...
def update_vacuum_position(new_row, new_col):
    global result
    update_cell(result['start_position'][0],
                result['start_position'][1], bg_image)
    result['start_position'][0] = new_row
    result['start_position'][1] = new_col
    update_cell(result['start_position'][0],
                result['start_position'][1], vacuum_image)
...
def clean_grid(aaa):
    ...
    step_label = tk.Label(window, text="Step: 0",
                          font=("Arial", 12, "bold"))
    step_label.grid(row=1, column=0, columnspan=2, padx=20, pady=10)

    def update_step(step):
        step_label.config(text="Step: " + str(step))
    if (aaa == 1):
        ...
        for dust in dust_positions:
            ...
            path = find_path_to_closest_goal(
                result['matrix'], (vacuum_pos[0], vacuum_pos[1]), dust_positions)
            count += len(path)
        ...
        if (aaa == 4):
            ...
            path = initialize_SLAM(result["matrix"], result["start_position"])
        # Di chuyển qua từng bước trên đường đi
        for step in path:
            move_vacuum(vacuum_pos[0], vacuum_pos[1], step[0], step[1])
            update_step(len(path))
            window.update()
...
def start_cleaning_slam():
    print("Start Cleaning SLAM full")
    # Bắt đầu quá trình dọn dẹp
    clean_grid(6)
...
def run_application():
    ...
    info_frame = tk.LabelFrame(window1, text="Nhập thông tin", font=(
        "Arial", 12, "bold"), padx=20, pady=20)
    ...
    obstacle_label = tk.Label(
        info_frame, text="Nhập số vật cản:", font=label_font)
    ...
    # Tạo nút "Reset"
    reset_button = tk.Button(window1, text="Reset", font=("Arial", 12), bg="#f44336", fg="white", relief=tk.RAISED,
                             command=create_table_prev)
    # reset_button.pack(pady=10)
    reset_button.grid(row=5, column=0, columnspan=2, pady=10)

    start_cleaning_A_star_button = tk.Button(
        window1, text="Start Cleaning A*", command=start_cleaning_A_star)
    start_cleaning_A_star_button.grid(row=7, column=0, columnspan=2, pady=10)
    start_cleaning_bfs_button = tk.Button(
        window1, text="Start Cleaning bfs", command=start_cleaning_bfs)
    start_cleaning_bfs_button.grid(row=8, column=0, columnspan=2, pady=10)
    start_cleaning_dfs_button = tk.Button(
        window1, text="Start Cleaning dfs", command=start_cleaning_dfs)
    start_cleaning_dfs_button.grid(row=9, column=0, columnspan=2, pady=10)
    start_cleaning_b_button = tk.Button(
        window1, text="Start Cleaning BFS full", command=start_cleaning_b)
    start_cleaning_b_button.grid(row=10, column=0, columnspan=2, pady=10)
    start_cleaning_d_button = tk.Button(
        window1, text="Start Cleaning DFS full", command=start_cleaning_d)
    start_cleaning_d_button.grid(row=11, column=0, columnspan=2, pady=10)
    start_cleaning_slam_button = tk.Button(
        window1, text="Start Cleaning SLAM", command=start_cleaning_slam)
    start_cleaning_slam_button.grid(row=12, column=0, columnspan=2, pady=10)
